...
class TestPerformance(unittest.TestCase):
    ...
    def test_batch_analysis_performance(self):
        ...
        self.assertGreater(
            throughput, 500, f"Throughput too low: {throughput:.2f} req/s"
        )
    ...
    def test_concurrent_analysis_performance(self):
        ...
        self.assertLess(
            total_time, 1.0, f"Concurrent execution too slow: {total_time:.3f}s"
        )
...
HTTP response body: {{"message":"{message}"}}"""
        ...
        # ログパース時間が適切であることを確認
        self.assertLess(
            avg_time, 0.005, f"Log parsing too slow: {avg_time:.6f}s per log"
        )
