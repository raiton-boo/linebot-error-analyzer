...
from linebot_error_analyzer.models import (
    ErrorResult, ApiPattern, ErrorCategory, 
    LogParseResult, ErrorInfo
)
...
class TestTypes(unittest.TestCase):
    ...
    def test_api_pattern_enum(self):
        ...
        # 全パターンの存在確認
        expected_patterns = [
            'MESSAGE_REPLY', 'MESSAGE_PUSH', 'MESSAGE_MULTICAST',
            'USER_PROFILE', 'GROUP_PROFILE', 'ROOM_PROFILE',
            'GROUP_MEMBER_PROFILE', 'ROOM_MEMBER_PROFILE',
            'GROUP_MEMBER_COUNT', 'ROOM_MEMBER_COUNT',
            'RICH_MENU_CREATE', 'RICH_MENU_UPDATE', 'RICH_MENU_DELETE',
            'WEBHOOK_SETTINGS', 'CHANNEL_ACCESS_TOKEN'
        ]
        
        for pattern_name in expected_patterns:
            self.assertTrue(hasattr(ApiPattern, pattern_name), 
    ...
    def test_error_category_enum(self):
        ...
        expected_categories = [
            'CLIENT_ERROR', 'AUTHENTICATION_ERROR', 'AUTHORIZATION_ERROR',
            'RESOURCE_NOT_FOUND', 'USER_BLOCKED', 'RATE_LIMIT_EXCEEDED',
...
HTTP response headers: HTTPHeaderDict({'key1': 'value1', 'key2': 'value2'})"""
    ...
    def test_optional_fields_handling(self):
        ...
        if hasattr(result_without_id, 'request_id'):
            ...
            self.assertTrue(result_without_id.request_id is None or 
                           isinstance(result_without_id.request_id, str))
